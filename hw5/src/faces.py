...
def cheat_init(points) :
    ...
    for i in range(len(labels)):
        clusters.add(Cluster([pt for pt in points if pt.label == labels[i]]))
...
def kAverage(points,k,average,init="random",plot=True):
    ...
    while(True):
        #find new assignments for all points
        rd +=1
        assignments = {}
        for p in points:
            min_dist = 100000000
            for idx in  range(k):
                dis = p.distance(centers[idx])
                if(dis < min_dist):
                    assignments[p] = idx
                    min_dist = dis
        ...
        for i in range(k):
            new_clusters.add(Cluster([key for key,value in list(assignments.items()) if value == i]))
        if plot:
            plot_clusters(new_clusters,"Interation {}".format(rd),ClusterSet.centroids)
        #compute new centroids
        centers = average(new_clusters)
        print("center for iteration{}: {}".format(rd,[center.attrs for center in centers]))
...
def kMeans(points, k, init='random', plot=False) :
    ...
    k_clusters = kAverage(points,k,ClusterSet.centroids,init,plot)
    return k_clusters
    ### ========== TODO : END ========== ###
...
def kMedoids(points, k, init='random', plot=False) :
    ...
    ### ========== TODO : START ========== ###
    # part e: implement
    k_clusters = kAverage(points,k,ClusterSet.medoids,init,plot)
    return k_clusters
    ### ========== TODO : END ========== ###
