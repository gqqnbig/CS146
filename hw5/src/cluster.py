...
class Cluster(object) :
    ...
    def centroid(self) :
        ...
        # print "sum is {}".format(attr)
        # print "length of self.points: {}".format(len(self.points))
        attr = np.divide(attr,float(len(self.points)))
        # print "centroids are: {}".format(attr)
        cluster_label, count = stats.mode(labels)
    ...
    def medoid(self) :
        ...
        for p in self.points:
            dist = 0
            for neibor in self.points:
                dist += p.distance(neibor)
            if dist < min_dist:
                medoid = p
                min_dist = dist
        return medoid
